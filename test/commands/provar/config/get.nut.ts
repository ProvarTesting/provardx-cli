...
describe('sf provar config get NUTs', () => {
  ...
  it('Missing property error as property name is missing', () => {
    ...
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE}`
    ).shellOutput;
  ...
  it('Missing property error in json format as property name is missing', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} --json`
    ).jsonOutput;
  ...
  it('Unknown Property error as property is not present in the file', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} metadata.test`
    ).shellOutput;
  ...
  it('Unknown Property error as property is not present in the file', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} environment.webBrowserProvider`
    ).shellOutput;
    expect(getOutput.stderr).to.deep.equal(`Error (1): [UNKNOWN_PROPERTY] ${errorMessages.UNKNOWN_PROPERTY}\n\n`);
  });
  ...
  it('Unknown Property error as property is not present in the file', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} projectPath.web`
    ).shellOutput;
    expect(getOutput.stderr).to.deep.equal(`Error (1): [UNKNOWN_PROPERTY] ${errorMessages.UNKNOWN_PROPERTY}\n\n`);
  });
  ...
  it('Unknown Property error in json format as property is not present in the file', () => {
    const result = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} sample --json`,
      {
        ensureExitCode: 0,
      }
    );
  ...
  it('value should be returned for provarHome property', () => {
    ...
    execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_SET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} provarHome=${setProvarHome}`
    );
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} provarHome`
    ).shellOutput;
  ...
  it('value should be returned for projectPath property', () => {
    ...
    const output = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} projectPath`
    ).shellOutput;
    expect(output.stdout).to.deep.equal('${PROVAR_PROJECT_PATH}\n');
    execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_SET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} projectPath=${setProjectPath}`
    );
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "projectPath"`
    ).shellOutput;
  ...
  it('value should be returned for resultsPath property in json format', () => {
    execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_SET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "resultsPath"="C:/Users/anchal.goel/git/ProvarRegressionqam5/test/provardx/Results"`
    );
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} resultsPath --json`,
      {
        ensureExitCode: 0,
      }
    );
  ...
  it('value should be returned for smtpPath property', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "smtpPath"`
    ).shellOutput;
    expect(getOutput.stdout).to.deep.equal('\n');
  });
  it('value should be returned for testOutputLevel property', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} testOutputLevel`
    ).shellOutput;
  ...
  it('value should be returned for pluginOutputlevel property', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} pluginOutputlevel`
    ).shellOutput;
  ...
  it('value should be returned for resultsPathDisposition property in json format', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "resultsPathDisposition" --json`
    ).jsonOutput;
  ...
  it('value should be returned for lightningMode property in json format', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} lightningMode --json`
    ).jsonOutput;
    expect(getOutput?.result.success).to.deep.equal(true);
  ...
  it('Value should be returned successfully for metdata object', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} metadata`
    ).shellOutput;
  ...
  it('Value should be returned successfully for environment object in json format', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} environment --json`,
      {
        ensureExitCode: 0,
      }
    );
  ...
  it('Value should be returned successfully for testEnvironment property in environment object', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} environment.testEnvironment`
    ).shellOutput;
  ...
  it('Value should be returned successfully for webBrowserDeviceName property in environment object', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "environment"."webBrowserDeviceName"`
    ).shellOutput;
  ...
  it('Value should be returned successfully for cachePath property in metadata object', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} metadata.cachePath --json`
    ).jsonOutput;
    expect(getOutput?.result.success).to.deep.equal(true);
  ...
  it('Value should be returned successfully for testprojectSecrets property', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} testprojectSecrets`
    ).shellOutput;
  ...
  it('Value should be returned successfully for connectionRefreshType property', () => {
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} connectionRefreshType`
    ).shellOutput;
  ...
  it('value should be returned for new added property', () => {
    execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_SET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "Test Suite"="Multiple Suites"`
    );
    const getOutput = execCmd<SfProvarCommandResult>(
      `${commandConstants.SF_PROVAR_CONFIG_GET_COMMAND} -f ${FILE_PATHS.VALUES_FILE} "Test Suite"`
    ).shellOutput;
