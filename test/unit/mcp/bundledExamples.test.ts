...
describe('bundled examples', () => {
  ...
  it('ranks a keyword-matching example above zero-overlap queries', () => {
    const onTopic = selectBundledExamples('ui account create validate', 1)[0];
    const offTopic = selectBundledExamples('zzz nothing matches here', 1)[0];
    assert.ok(onTopic.similarity_score > offTopic.similarity_score);
  });
});
