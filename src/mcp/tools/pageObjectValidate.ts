...
export function registerPageObjectValidate(server: McpServer, config: ServerConfig): void {
  server.tool(
    ...
    ({ content, file_path, expected_class_name }) => {
      ...
      log('info', 'provar_pageobject_validate', { requestId, has_content: !!content, file_path });
      ...
      try {
        ...
        log('error', 'provar_pageobject_validate failed', { requestId, error: error.message });
...
export function validatePageObject(source: string, expectedClassName?: string): PageObjectValidationResult {
...
function validateFields(fields: FieldInfo[], hasSalesforceAnnotation: boolean, issues: ValidationIssue[]): void {
...
function checkLocatorQuality(field: FieldInfo, hasSalesforceAnnotation: boolean, issues: ValidationIssue[]): void {
