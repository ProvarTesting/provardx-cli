...
export interface ProjectContext {
   ...
   * Populated by provar_project_inspect → secrets_validation.unencrypted_key_count.
...
const RULE_REGISTRY: Record<string, { name: string; description: string }> = {
  'SUITE-EMPTY-001': {
    ...
    description: 'A test suite must contain at least one test case or child suite.',
  },
  'SUITE-DUP-001': {
    name: 'Duplicate Test Case Name in Suite',
    description: 'Test case names within a suite must be unique.',
  },
  'SUITE-DUP-002': {
    name: 'Duplicate Child Suite Name',
    description: 'Child suite names within a parent suite must be unique.',
  },
  'SUITE-SIZE-001': {
    name: 'Oversized Test Suite',
    description: 'A test suite should contain no more than 75 test cases.',
  },
  'SUITE-NAMING-001': {
    name: 'Inconsistent Child Suite Naming',
    description: 'All child suite names should follow a consistent naming convention.',
  },
  'SUITE-NAMING-002': {
    name: 'Inconsistent Test Case Naming',
    description: 'All test case names within a suite should follow a consistent naming convention.',
  },
  'PLAN-EMPTY-001': { name: 'Empty Test Plan', description: 'A test plan must contain at least one test suite.' },
  'PLAN-DUP-001': {
    name: 'Duplicate Suite Name in Plan',
    description: 'Suite names within a test plan must be unique.',
  },
  'PLAN-META-001': {
    name: 'Missing Plan Objectives',
    description:
      'Test plans should define clear testing objectives. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-002': {
    name: 'Missing In-Scope Definition',
    description:
      'Test plans should specify which features are in scope. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-003': {
    name: 'Missing Testing Methodology',
    description:
      'Test plans should document the testing methodology. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-004': {
    name: 'Missing Acceptance Criteria',
    description:
      'Test plans should define acceptance criteria or an acceptable pass rate. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-005': {
    name: 'Missing Test Environments',
    description:
      'Test plans should specify the target test environments. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-006': {
    name: 'Missing Test Data Strategy',
    description:
      'Test plans should document how test data will be prepared and cleaned up. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-META-007': {
    name: 'Missing Risk Assessment',
    description:
      'Test plans should identify potential risks and mitigations. This field is configured in the Provar Quality Hub app (not stored in local project files).',
  },
  'PLAN-SIZE-001': {
    name: 'Oversized Test Plan',
    description: 'A test plan should contain no more than 20 test suites.',
  },
  'PLAN-NAMING-001': {
    name: 'Inconsistent Suite Naming in Plan',
    description: 'All suite names within a test plan should follow a consistent naming convention.',
  },
  'PROJ-EMPTY-001': { name: 'Empty Project', description: 'A project must contain at least one test plan.' },
  'PROJ-DUP-001': {
    name: 'Duplicate Test Case Name Across Project',
    description: 'Test case names must be unique across the entire project.',
  },
  'PROJ-DUP-002': {
    name: 'Duplicate Plan Name in Project',
    description: 'Test plan names within a project must be unique.',
  },
  'PROJ-CALLABLE-001': {
    name: 'Unresolved caseCall Reference',
    description: 'All caseCall references must point to existing test cases in the project.',
  },
  'PROJ-CALLABLE-002': {
    name: 'Missing Callable Test Case',
    description: 'All callable tests referenced by caseCall must exist in the project.',
  },
  'PROJ-CONN-001': {
    name: 'Undefined Connection Name',
    description: 'All connection names used in test cases must be defined in the project context.',
  },
  'PROJ-ENV-001': {
    name: 'Duplicate Environment Name',
    description: 'Environment names within a project must be unique.',
  },
  'PROJ-ENV-002': {
    name: 'Invalid Environment Name',
    description: 'Environment names must contain only letters, digits, and underscores.',
  },
  'PROJ-SECRET-001': {
    name: 'Missing Secrets Password',
    description: 'A Provar Secrets Password must be configured to protect sensitive test data.',
  },
  'PROJ-ENC-001': {
    name: 'Unencrypted Credentials in .secrets',
    description:
      'All values in the .secrets file must be wrapped in ENC1() encryption. Plaintext credentials are a critical security risk.',
  },
};
...
export function validateHierarchyTestCase(tc: TestCaseInput, qualityThreshold: number): TestCaseResult {
...
export function validateSuite(data: TestSuiteInput, qualityThreshold: number): SuiteResult {
  ...
  const violations: HierarchyViolation[] = [
    ...
    ...checkNamingConsistency(
      childSuites.map((cs) => cs.name),
      `suite "${suiteName}" child suites`,
      'TestSuite',
      'SUITE-NAMING-001'
    ),
    ...checkNamingConsistency(
      childCases.map((tc) => tc.name),
      `suite "${suiteName}" test cases`,
      'TestSuite',
      'SUITE-NAMING-002'
    ),
  ];
  ...
  const scores = [...caseResults.map((r) => r.quality_score), ...suiteResults.map((r) => r.quality_score)];
  ...
  if (!meta.objectives)
    v.push(
      makeViolation(
        ...
        'info',
        1,
        `Test plan "${planName}" has no objectives defined`,
        'Add testing objectives via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.in_scope)
    v.push(
      makeViolation(
        ...
        'info',
        1,
        `Test plan "${planName}" has no in-scope definition`,
        'Specify in-scope features via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.testing_methodology)
    v.push(
      makeViolation(
        'PLAN-META-003',
        'PlanCompleteness',
        'info',
        1,
        `Test plan "${planName}" has no testing methodology defined`,
        'Document the testing methodology via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.acceptance_criteria && meta.acceptable_pass_rate === undefined)
    v.push(
      makeViolation(
        'PLAN-META-004',
        'PlanCompleteness',
        'info',
        1,
        `Test plan "${planName}" has no acceptance criteria or acceptable pass rate`,
        'Set acceptance criteria or an acceptable pass rate via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.environments?.length)
    v.push(
      makeViolation(
        ...
        'info',
        1,
        `Test plan "${planName}" has no test environments defined`,
        'Specify target environments (e.g., QA, Staging, UAT) via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.test_data_strategy)
    v.push(
      makeViolation(
        'PLAN-META-006',
        'PlanCompleteness',
        'minor',
        2,
        `Test plan "${planName}" has no test data strategy`,
        'Document the test data strategy via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  if (!meta.risks)
    v.push(
      makeViolation(
        ...
        'info',
        1,
        `Test plan "${planName}" has no risks identified`,
        'Identify risks and mitigations via the Provar Quality Hub app',
        ['TestPlan']
      )
    );
  return v;
...
export function validatePlan(data: TestPlanInput, qualityThreshold: number): PlanResult {
    ...
    if (seenSuites.has(lower)) {
      violations.push(
        makeViolation(
          ...
          'major',
          5,
          `Duplicate suite name "${s.name}" in plan "${planName}"`,
          'Ensure all suite names within a plan are unique',
          ['TestPlan']
        )
      );
    }
  ...
  if (suiteCount > 20) {
    violations.push(
      makeViolation(
        ...
        'minor',
        2,
        `Test plan "${planName}" contains ${suiteCount} test suites (recommended maximum: 20)`,
        'Split the plan into multiple focused test plans',
        ['TestPlan']
      )
    );
  }
  ...
  const scores = [...suiteResults.map((r) => r.quality_score), ...caseResults.map((r) => r.quality_score)];
  const quality_score = scores.length
    ? Math.max(0, Math.round((childAvg(scores) - computeViolationDeduction(violations)) * 100) / 100)
    : 0;
    ...
    if (seen[base]) {
      violations.push(
        makeViolation(
          ...
          'major',
          5,
          `Duplicate test case name detected: "${base}"`,
          'Ensure all test case names are unique across the project',
          ['Project'],
          [seen[base], name]
        )
      );
...
function checkCaseCalls(
  registry: Record<string, string>,
  diskTestCaseNames: Set<string> = new Set()
): HierarchyViolation[] {
  const violations: HierarchyViolation[] = [];
  ...
  const availableBase = new Set([
    ...Object.keys(registry).map((n) => path.basename(n, path.extname(n))),
    ...diskTestCaseNames,
  ]);
  ...
  for (const [testName, xml] of Object.entries(registry)) {
    ...
    try {
      ...
      const calls = rawCalls
        ? ((Array.isArray(rawCalls) ? rawCalls : [rawCalls]) as Array<Record<string, unknown>>)
        : [];
    ...
    if (!availableBase.has(called)) {
      violations.push(
        makeViolation(
          ...
          'critical',
          10,
          `Callable test "${called}" is referenced but does not exist in project`,
          `Add "${called}.testcase" to the project or remove the caseCall references`,
          ['Project']
        )
      );
    }
  ...
  for (const [conn, files] of Object.entries(undefinedConns)) {
    violations.push(
      makeViolation(
        ...
        'major',
        5,
        `Connection "${conn}" used but not defined in project`,
        `Ensure connection "${conn}" is defined in your Provar project`,
        ['Project'],
        files
      )
    );
    ...
    if (seen.has(lower)) {
      violations.push(
        makeViolation(
          ...
          'major',
          5,
          `Duplicate environment name: "${env}"`,
          'Remove or rename duplicate environments',
          ['Project']
        )
      );
    }
    ...
    if (!envPattern.test(env)) {
      violations.push(
        makeViolation(
          ...
          'major',
          5,
          `Environment name "${env}" contains invalid characters`,
          'Use only letters, digits, and underscores for environment names',
          ['Project']
        )
      );
    }
  ...
  if (!testPlans.length && !testSuites.length && !testCases.length) {
    violations.push(
      makeViolation(
        ...
        'major',
        5,
        `Project "${projectName}" contains no test plans, test suites, or test cases`,
        `Add at least one test plan, test suite, or test case to "${projectName}"`,
        ['Project']
      )
    );
  }
    ...
    if (seenPlans.has(lower)) {
      violations.push(
        makeViolation(
          ...
          'major',
          5,
          `Duplicate plan name "${p.name}" in project "${projectName}"`,
          'Ensure all test plan names within a project are unique',
          ['Project']
        )
      );
    }
  ...
  if (!ctx.secretsPasswordSet) {
    violations.push(
      makeViolation(
        ...
        'major',
        5,
        'Provar Secrets Password is not configured for this project',
        'Set a Secrets Password in the Provar project settings to encrypt sensitive test data',
        ['Project']
      )
    );
  }
...
export function buildHierarchySummary(result: SuiteResult | PlanResult | ProjectResult): HierarchySummary {
  ...
  function walkNode(node: SuiteResult | PlanResult | ProjectResult | TestCaseResult): void {
    ...
    if (nodeLevel in stats.violations_by_level)
      stats.violations_by_level[nodeLevel as keyof typeof stats.violations_by_level] += nodeViolations.length;
