    ...
    'provar_testsuite_validate',
    {
      ...
      inputSchema: {
        ...
        baseline_run_id: z
          ...
          .describe(
            desc(
              'run_id from a previous call. When provided, returns only violations that are new or resolved since that run: { added, resolved, unchanged, unchanged_count, run_id }. `unchanged` carries findings still present with their CURRENT message and details, so a partially-fixed aggregate finding still tells you what remains. If not found, returns error BASELINE_NOT_FOUND.',
