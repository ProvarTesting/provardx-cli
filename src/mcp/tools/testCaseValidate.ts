...
export function validateTestCase(
  ...
  validateConnectionReferenceConsistency(tc, issues);
...
function checkUiInteraction(call: Record<string, unknown>, stepName: string, issues: ValidationIssue[]): void {
  ...
  // The valid UiDoAction interactions are `action` (click a button/link), `set`
  // (fill a field or pick a picklist value) and `file` (upload). Common LLM
  // hallucinations like `click`/`type`/`select` render the IDE Action widget
  // blank and do not run. Denylist the known-wrong names (mapping each to the
  // correct one) rather than allow-listing, so a valid-but-rare name never
  // false-fires.
...
function validateConnectionReferenceConsistency(tc: Record<string, unknown>, issues: ValidationIssue[]): void {
  const calls: Array<Record<string, unknown>> = [];
  collectNodesByKey(tc, 'apiCall', calls);
  ...
  if (connectStepCount === 0) return; // connections may be inherited from a parent test
    ...
    for (const refId of CONNECT_REFERENCE_ARG_IDS) {
      ...
      if (ref && !available.has(ref)) {
        ...
        issues.push({
          ...
          severity: 'WARNING',
          message: `Step "${stepName}" references ${refId}="${ref}", but no connect step in this test produces that connection result (available: ${
            [...available].map((n) => `"${n}"`).join(', ') || 'none'
          }).`,
          applies_to: 'apiCall',
          suggestion: `Set the connect step's resultName to "${ref}", or change ${refId} to match a connect step's resultName. Downstream steps reference the connect step's resultName — which defaults to the connectionName when resultName is omitted.`,
        });
