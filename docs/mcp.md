...
| `grouping_mode`       | `auto` \| `flat` \| `single-screen`      | no       | Controls how `UiDoAction` / `UiAssert` / `UiRead` siblings are nested under a preceding `UiWithScreen`. Default `auto`.   |
...
**`grouping_mode` — auto-nesting UI actions under `UiWithScreen`.** Provar IDE renders a test case correctly only when `UiDoAction`, `UiAssert`, and `UiRead` steps live inside their owning `UiWithScreen`'s `<clauses><clause name="substeps"><steps>…</steps></clause></clauses>` block. When the caller passes a flat `steps[]` payload, the generator auto-groups trailing UI-action siblings into that block.

| Mode             | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `auto` (default) | When a `UiWithScreen` is followed by `UiDoAction` / `UiAssert` / `UiRead` siblings, those siblings are absorbed into the screen's `<clause name="substeps">` block. The grouping run stops at the next `UiWithScreen`, any non-UI step (`SetValues`, `ApexConnect`, …), or end of list. `testItemId`s are assigned depth-first: parent screen, then its substeps slot, then its children. Numbering remains sequential and gap-free. |
